//Foundation
#include "CbtChainOfResponsibility.hpp"
#include "OperatingSystemModule.hpp"

namespace Fnd {

    ChainOfResponsibility::ChainOfResponsibility() {
            ErrorType error = OperatingSystem::Instance().createSemaphore(1, 1, binaryQueueSemaphore);
    ...
    ErrorType ChainOfResponsibility::addCommandObject(std::unique_ptr<CommandObject> &commandObject) {
        ...
        ErrorType error = OperatingSystem::Instance().waitSemaphore(binaryQueueSemaphore, SemaphoreTimeout);
        ...
        if (_commandObjects[logicSignature].size() >= MaxCommandObjectSize) {
            error = OperatingSystem::Instance().incrementSemaphore(binaryQueueSemaphore);
            assert(ErrorType::Success == error);
    ...
    std::unique_ptr<CommandObject> ChainOfResponsibility::getNextCommand(LogicSignature signature, ErrorType &error) {
        error = OperatingSystem::Instance().waitSemaphore(binaryQueueSemaphore, SemaphoreTimeout);
        if (ErrorType::Success != error) {
